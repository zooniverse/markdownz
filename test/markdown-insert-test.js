import markdownInsert from '../src/lib/markdown-insert';
...
describe('markdownInsert', () => {
  ...
  describe('#hrefLink', () => {
    beforeEach(() => {
      ({ text } = markdownInsert.hrefLink('google', 'https://google.com'));
    });
    ...
    it('should set the link href to the first argument', () => {
      expect(text).to.match(/\(https:\/\/google.com\)/);
    });

    it('should set the link name to the second argument', () => {
  ...
  describe('#imageLink', () => {
    beforeEach(() => {
      ({ text } = markdownInsert.imageLink('https://google.com/test.png', 'logor'));
    });
  ...
  describe('#videoLink', () => {
    beforeEach(() => {
      ({ text } = markdownInsert.videoLink('https://youtu.be/cjC94EhAs00', 'youtube'));
    });
  ...
  describe('#bold', () => {
    it('should wrap the provided string in "**"', () => {
      ({ text } = markdownInsert.bold('blahblah'));
  ...
  describe('#italic', () => {
    it('should wrap the provided string in "*"', () => {
      ({ text } = markdownInsert.italic('blahblah'));
      expect(text).to.match(/^\*[A-z]+\*$/);
    });
  });
  ...
  describe('#quote', () => {
    it('should prefix the provided string with "> "', () => {
      ({ text } = markdownInsert.quote('blahblah'));
  ...
  describe('#bullet', () => {
    it('should prefix the provided string with "- "', () => {
      ({ text } = markdownInsert.bullet('blahblah'));
      expect(text).to.match(/^-\s[A-z]+$/);
    });
  });

  describe('#numberedList', () => {
    it('should prefix the provided string with "1. "', () => {
      ({ text } = markdownInsert.numberedList('blahblah'));
  ...
  describe('#heading', () => {
    it('should wrap the provided string in "*"', () => {
      ({ text } = markdownInsert.heading('blahblah'));
  ...
  describe('#numberedList', () => {
    it('should prefix the provided string with "----------\\n "', () => {
      ({ text } = markdownInsert.horizontalRule('blahblah'));
  ...
  describe('#strikethrough', () => {
    it('should wrap the provided string in "~~"', () => {
      ({ text } = markdownInsert.strikethrough('blahblah'));
      expect(text).to.match(/^~~[A-z]+~~$/);
    });
  });
  ...
  describe('#insertAtCursor', () => {
    ...
    it('should call focus', () => {
      const focusSpy = spy.on(input, 'focus');
      markdownInsert.insertAtCursor('**BOLDMOVE**', input, { start: 1, end: 21 });
  ...
  describe('#incrementListItems', () => {
    it('should number multiple lines in order', () => {
      const prevLines = 'One\nTwo\nThree';
      const lines = '1. One\n1. Two\n1. Three';
      expect(markdownInsert.incrementedListItems(prevLines, lines)).to.equal('1. One\n2. Two\n3. Three');
    });
  });
});
